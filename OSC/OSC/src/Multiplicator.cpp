...
Number Multiplicator::process(Number left, Number right) {
	...
	auto matrix = ocas::make_multiply_matrix(left, right);

	std::cout << "After building matrix" << std::endl;
	std::cout << matrix << std::endl << std::endl;

	auto vector = ocas::add_matrix(matrix);

	std::cout << "After creating vector out of matrix" << std::endl;
	std::cout << vector << std::endl << std::endl;

	auto solution = ocas::make_number(vector, left_decimal_places + right_decimal_places);
